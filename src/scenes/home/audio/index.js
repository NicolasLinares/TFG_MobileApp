...
import { deleteAudioHistory, updateDescription } from '_redux_actions';
...
class AudioScreen extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      localpath: this.props.navigation.state.params.localpath,
    ...
    return await fetch(URL.updateDescription + this.state.uid, 
      ...
      .then(([body, status]) => {
        if (status == 201) {
          ...
          this.props.navigation.state.params.description = this.state.description;
